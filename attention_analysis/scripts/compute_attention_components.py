...
def load_logits(input):
    all_logits = torch.load(input, map_location='cpu')
    #all_logits = tuple(x.cpu() if torch.is_tensor(x) else x for x in all_logits)
    return all_logits

# def testing_consistency_albert(input):
...
#     correlations = testing_consistency(all_logits)
#     return correlations

# def testing_consistency_prottrans(input):
#     all_logits = torch.load(input)
#     all_logits = all_logits.cpu()
#     all_logits = all_logits.permute(1, 0, 2 ,3,4)
#     all_logits = all_logits.numpy()
...
# def testing_consistency_t5(input):
#     all_logits = torch.load(input)
#     all_logits = all_logits.cpu()
...
# def testing_consistency_bert(input):
#     all_logits = load_logits(input)
#     all_logits = torch.cat(all_logits, dim=0)
#     all_logits = all_logits.numpy()
#     correlations = []
#     for layer_id, layer_logits in tqdm(enumerate(all_logits)):
#         for head_i, head_logits in enumerate(layer_logits):
#             a, b, c = disentangle_matrix(head_logits)
#             reconstructed_logits = get_low_rank_attention(a,b,c)
#             corr, _ = pearsonr(head_logits.flatten(), reconstructed_logits.flatten())
#             correlations.append(corr)
#     return correlations

# def testing_consistency_xlnet(input):
#     all_logits = load_logits(input)
#     all_logits = torch.stack(all_logits, dim=0)
#     all_logits = all_logits.squeeze(1)
...
# def process_tensor_file_prottrans(input, output):
#     #T5-3b: 24 attention heads, dimensions ([24, 1, 32, 17, 17])
#     all_logits = torch.load(input).cpu()
#     all_logits = all_logits.permute(1, 0, 2 ,3,4)
#     all_logits = all_logits.numpy()
...
#         for head_i, head_logits in enumerate(layer_logits):
#             a, b, c = disentangle_matrix(head_logits)
#             layer_components.append([a, b, c])  # Keep as list to handle different shapes
#         layer_components = np.array(layer_components, dtype=object)
#         all_components.append(layer_components)
#     all_components = np.array(all_components, dtype=object)
...
# def process_tensor_file_xlnet(input, output):
#     all_logits = load_logits(input)
#     all_logits = torch.stack(all_logits, dim=0)
#     all_logits = all_logits.squeeze(1)
#     #print(all_logits.shape)
...
# def process_tensor_file_t5(input, output, visualize=False, output_visual_dir=None):
...
# def process_tensor_file_bert(input, output, visualize=False, output_visual_dir=None):
#     #print("processing bert tensor")
...
# def process_tensor_file_albert(input, output):
#    # print("processing bert tensor")
#     #BERT: 24 layers, 16 attention heads
#     # length 24 ([1, 16, 512, 512])
...
# def process_tensor_file_esm(input, output):
#     all_logits = load_logits(input)
#     all_logits = all_logits[0]
#     all_logits = all_logits.numpy()
#     all_components = []
...
def process_folder(output_dir, input_dir, model, consistency_csv_path):
    ...
    for file_name in os.listdir(input_dir):
        ...
        # if (model in ["Rostlab/prot_t5_xl_bfd", "Rostlab/prot_t5_xl_uniref50", "Rostlab/prot_bert", "Rostlab/prot_xlnet", "Rostlab/prot_albert"]):
        #     process_tensor_file_prottrans(input_file, output_file)
        # elif (model == "t5-3b"):
        #     process_tensor_file_t5(input_file, output_file)
        # elif (model == "google-bert/bert-base-uncased"):
        #     process_tensor_file_bert(input_file, output_file)
        # elif (model == "albert-xxlarge-v2"):
        #     process_tensor_file_albert(input_file, output_file)
        # elif (model == "xlnet/xlnet-large-cased"):
        #     process_tensor_file_xlnet(input_file, output_file)
        # elif (model in ["esm"]):
        #     process_tensor_file_esm(input_file, output_file)
    return
...
def main(args):
    ...
    import csv
    results = {}
    if args.model_list == "A":
        model_list = ["google-bert/bert-base-uncased", "Rostlab/prot_bert"]
    elif args.model_list == "B":
        model_list = ["albert-xxlarge-v2" , "Rostlab/prot_albert"]
    elif args.model_list == "C":
        model_list = ["Rostlab/prot_t5_xl_bfd", "Rostlab/prot_t5_xl_uniref50", "t5-3b"]
    elif args.model_list == "D":
        model_list = ["Rostlab/prot_xlnet", "xlnet/xlnet-large-cased"]
    else:
    ...
    if args.run == "testing_consistency":
        with open(f"{args.consistency_file}.csv", 'a') as f:
            writer = csv.writer(f)
            for model in model_list:
                input_dir = os.path.join(input_dir, model_dirname)
                results[model] = []
                ...
                for file_name in selected_files:
                    input_file = os.path.join(input_dir, file_name)
                    if model in ["Rostlab/prot_t5_xl_bfd", "Rostlab/prot_t5_xl_uniref50", "Rostlab/prot_bert", "Rostlab/prot_xlnet", "Rostlab/prot_albert"]:
                        correlation = testing_consistency_prottrans(input_file)
                    elif (model == "google-bert/bert-base-uncased"):
                        correlation = testing_consistency_bert(input_file)
                    elif (model == "albert-xxlarge-v2"):
                        correlation = testing_consistency_albert(input_file)
                    elif (model == "t5-3b"):
                        correlation = testing_consistency_t5(input_file)
                    elif (model == "xlnet/xlnet-large-cased"):
                        correlation = testing_consistency_xlnet(input_file)
                    results[model].append(correlation)
                    mean_correlation = np.mean(correlation)
                    writer.writerow([model, file_name, mean_correlation, correlation])
                    f.flush()
        return 
    else:
        for model in model_list:
            ...
            process_folder(output_dir, input_dir, model)
    return
