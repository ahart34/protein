...
class Property_walltime_ProtBert(tasks.Task, core.Configurable):
    eps = 1e-10
    _option_members = {"task", "criterion", "metric"}
    ...
    def __init__(self, model, task=(), criterion="mse", metric=("mae", "rmse"), num_mlp_layer=2, #switched to 2
        ...
        super(Property_walltime_ProtBert, self).__init__()
        self.model = model
        self.task = task
        self.criterion = criterion
        self.metric = metric
...
@R.register("tasks.Property_Node_walltime_ProtBert_exit01")
class Property_Node_walltime_ProtBert_exit01(tasks.Task, core.Configurable):
    _option_members = {"criterion"}
    ...
    def __init__(
        ...
        super(Property_Node_walltime_ProtBert_exit01, self).__init__()
        self.model = model
        self.criterion = criterion
        self.metric = metric
        # For classification tasks, disable normalization
        ...
        with torch.no_grad():
            for lidx, layer in enumerate(self.model.encoder.layer):
                ...
                for loc, gidx in enumerate(active.tolist()):
                    ...
                    log_i = self.mlp[lidx](h_i)
                    logits_list.append(log_i)

                    probs = torch.sigmoid(log_i)
...
class EarlyExitClassificationTemperature_Node_continuous_ProtBert(tasks.Task, core.Configurable):
    ...
    def evaluate(self, preds, target):
        ...
        metric["avg_computed_layer"] = average_computed_layer.item()
        results = {}
        results["avg_layer"] = metric["layer"]
        results["avg_computed_layer"] = metric["avg_computed_layer"]
        results["macro_acc"] = metric["macro_acc"]
        return results
